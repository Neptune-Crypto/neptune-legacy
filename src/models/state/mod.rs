...
impl GlobalState {
    ...
    pub(crate) async fn restore_monitored_utxos_from_archival_mutator_set(&mut self) {
        ...
        // Otherwise, the function will proceed successfully but the resulting
        // membership proofs will be synced to some wrong block which is not
        // the tip.
        let asm_sync_label = ams_ref.get_sync_label();
        assert_eq!(
            tip_hash,
...
pub(crate) mod tests {
    ...
    use crate::models::state::wallet::wallet_status::WalletStatusExportFormat;
    ...
    pub(crate) mod helper {
        ...
        pub(crate) async fn send_coins(
            ...
            println!(
                ...
                !primitive_witness.kernel.public_announcements.is_empty()
            );
    ...
    async fn test_set_tip_to_stored_block() {
        ...
        // make transaction
        drop(alice_gsl); // drop lock to free up api
        let tx_b = helper::send_coins(&mut alice, NativeCurrencyAmount::coins(5), rando).await;
        alice_gsl = alice.lock_guard_mut().await; //re-acquire lock
