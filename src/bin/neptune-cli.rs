...
use neptune_cash::models::state::wallet::address::AddressableKeyType;
...
enum Command {
    ...
    ///
    /// Note that the resulting data dump contains private information that
    /// would enable the reader to deanonymize transactions. Only share this
    /// information if the privacy risk is acceptable.
    ///
    /// Furthermore, if the current node owns guesser reward UTXOs, this data
    /// dump contains the secret spending keys for those UTXOs. No other UTXO
    /// types are at risk of being spent.
    DumpDb {
        network: Network,
    ...
    /// Set the key counter, which is an integer which counts how many keys were
    /// generated by the wallet. The wallet is aware of this number of keys of
    /// the given type.
    ///
    /// Under normal circumstances, the wallet manages the key counters on its
    /// own. However, when importing wallets or after out-of-ordinary syncing,
    /// the counters may become unmatched with the UTXOs that the wallet is
    /// managing. In this case, the wallet will track UTXOs that it does not
    /// seem to know the spending key for. That error might be fixed by setting
    /// the key counters to a higher value.
    ///
    /// This is a dangerous command because if it is set too low, the wallet
    /// will become unaware of UTXOs it has under management. Conversely, if set
    /// too high, the wallet will waste precious computing resources managing
    /// keys that are never used.
    ///
    /// Arguments:
    ///
    ///  - keytype (required): "generation" or "symmetric"
    ///  - counter (required): any positive number
    ///
    /// Example: `> neptune-cli set-key-counter generation 20`
    ///
    /// Generation keys are associated with generation addresses, the typical
    /// long string starting with `nolga`. Symmetric keys are used for change
    /// and should not be sent to untrusted parties.
    SetKeyCounter {
        #[arg(value_parser = AddressableKeyType::from_str)]
...
async fn main() -> Result<()> {
    ...
    match args.command {
        ...
        Command::SetKeyCounter { keytype, counter } => {
            match client.set_key_counter(ctx, token, keytype, counter).await? {
                Ok(_) => {
                    println!("success.");
                }
                Err(server_error) => {
                    println!("failed: {server_error}");
                }
            };
        }
